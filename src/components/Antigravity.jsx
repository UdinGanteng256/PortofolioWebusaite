...
const AntigravityInner = ({
    ...
    useEffect(() => {
        if (meshRef.current) {
            ...
            for (let i = 0; i < count; i++) {
                dummyColor.set(colorList[i % colorList.length]);
                meshRef.current.setColorAt(i, dummyColor);
            }
            meshRef.current.instanceColor.needsUpdate = true;
        }
    }, [count, color, dummyColor]);

    const lastMousePos = useRef({ x: 0, y: 0 });
    const lastMouseMoveTime = useRef(0);
    const virtualMouse = useRef({ x: 0, y: 0 });

    // Track pointer globally so it always responds, even when Canvas sits behind pointerEvents elements
    ...
    return (
        <instancedMesh ref={meshRef} args={[undefined, undefined, count]}>
            ...
            <meshBasicMaterial toneMapped={true} vertexColors={false} />
        </instancedMesh>
    );
};
...
const Antigravity = props => {
    return (
        <Canvas camera={{ position: [0, 0, 50], fov: 35 }} style={{ width: '100%', height: '100%', display: 'block' }}>
